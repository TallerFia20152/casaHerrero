...
public class MySqlFactorySMatricula implements DAOFactorySMatricula {
	...
	public List<Curso> CursosPreferibles(String codigoAlumno) throws Exception {
		...
		try {
			...
			sql = new StringBuffer();
			sql.append(" SELECT p.curso_id,c.nombre,p.creditos,cic.nombre as ciclo  ");
			sql.append(" FROM t_pre_matricula_alumno al, t_curso c ,t_plan_curricular_detalle p,t_ciclo cic");
			sql.append(" where alumno_id='"+ codigoAlumno+ "'");
			sql.append(" And p.curso_id=c.id ");
			sql.append(" And p.ciclo_id=cic.id");
			sql.append(" And p.curso_id=al.curso_id");
		...
		try {
			...
			for (Alumno alumno : alumnoIdList) {
				...
				while (rs.next()) {
					...
					cursoAlumno.setCurso(rs.getString("nom_curso"));
					cursoAlumno.setCredito(rs.getString("creditos"));
					cursoAlumno.setCantidadAlumnos(1);
									...
									if (returnArea.getNombre().equalsIgnoreCase(area.getNombre())) {
										for (Curso cursoArea : area.getCursoList()) {
											if (curso.getCodigo() == cursoArea.getCodigo()
													&& curso.getSeccion().equalsIgnoreCase(Integer.toString())) {
												cursoArea.setCantidadAlumnos(cursoArea.getCantidadAlumnos() + 1);
												cursoInArea = true;
												break;
											}
										}
...
*/
	...
	private List<Area> ObtenerProbablesCursosCantidad() throws Exception {
		...
		try {
				...
				while (rs.next()) {
					...
					for(Alumno alumno:listaAlumno)
					{
						...
						for(Curso cursoProbableAlumno:listaCursoProbables)
						{
							...
							{
								curso.setSeccio(ObtenerSeccion(cursoProbableAlumno.getCodigo()));
								System.out.println("LA SECCION OBTENIDA ES " + curso.getSeccio());
			...
			System.out.println("ERROR AL OBTENER PROBABLES CURSOS CANTIDAD=> " + e.getMessage());
			return null;
		} finally {
			LimpiarConexion(con, query, ps, rs);
		}
	...
	private String ObtenerSeccion(String codCurso) throws Exception
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		StringBuffer query = null;
		...
		try {
			...
			query = new StringBuffer();
			query.append("SELECT c.id,c.nombre,s.nombre from t_curso_seccion cs,t_curso c,t_seccion s");
			query.append(" Where cs.curso_id=c.id");
			query.append(" And c.id= '" + codCurso+ "'");
			query.append(" And cs.seccion_id=s.id");
					
			System.out.println("SECCION ==> "+ query.toString());
	...
	private Integer CantidadAlumnoPreferencial(String codigoCurso) throws Exception {
		...
		try {
			...
			System.out.println("ERROR AL OBTENER CANTIDAD CURSOS PREFERENCIAL=> " + e.getMessage());
			return null;
		} finally {
			//LimpiarConexion(con, query, ps, rs);
		}
	...
	public List<Area> SimulacionCursosPreferidos() throws Exception 
	{
		...
		try {
			...
			System.out.println("ERROR AL OBTENER CURSOS PREFERIDOS=> " + e.getMessage());
			return null;
		} finally {
			LimpiarConexion(con, query, ps, rs);
		}
	...
	public boolean EliminarHorariosAlumno(String codAlumno) throws Exception {
		...
		try {
			...
			if (con == null)
				...
				ps = con.prepareStatement(sql.toString());
				int filaMatricula = ps.executeUpdate();
				System.out.println("FILA MATRICULA " +filaMatricula);
			return true;
	...
	public List<Seccion> SeleccionarSeccion(String idCurso) throws Exception {
		...
		{			
			query = new StringBuffer();
			query.append("SELECT c.id as codigo,c.nombre as curso,s.nombre as seccion,d.nombre as dia,h.horaInicio,max(h.horaFin) as horaFin,h.turno");
			query.append(" FROM t_plan_curricular_detalle pd, t_horario_detalle hd, t_disponibilidad_aula da,t_aula a, t_dia d,t_hora h,t_curso_seccion cs,t_curso c,t_seccion s");
			query.append(" Where da.id=hd.disponibilidad_aula_id");
			query.append(" And pd.curso_id=c.id");
			query.append(" And da.aula_id=a.id");
			query.append(" And da.dia_id=d.id");
			query.append(" And da.hora_id=h.id");
			query.append(" And hd.curso_seccion_id=cs.id");
			query.append(" And hd.disponibilidad_aula_id=da.id");			
			query.append(" And cs.curso_id=c.id");
			query.append(" and cs.seccion_id=s.id");
			query.append(" And c.id=" +idCurso );
			query.append(" GROUP BY s.id,d.nombre");
			query.append(" ORDER BY 3,2 desc,h.turno desc,horaInicio");
	...
	public boolean BuscarHorariosAlumnos(String codAlumno) throws Exception {
		...
		try {
			...
			query = new StringBuffer();
			query.append("SELECT count(*) as cantidad");
			query.append(" FROM t_horario_alumno");
			query.append(" WHERE alumno_id= '" + codAlumno+ "'");
	...
	public List<Curso> ListarReporteMatriculaProgrevisa() throws Exception {
		...
		try {
			...
			listado = new ArrayList<Curso>();
			/*
			sql.append(" SELECT c.id,c.nombre,s.nombre ");
			sql.append(" from t_curso_seccion cs,t_curso c,t_seccion s ");
			...
			*/
			...
			sql.append("select c.id as codigo,cs.abreviatura as curso,s.nombre as seccion");
			sql.append(" From t_curso c, t_curso_seccion cs , t_seccion s");
			sql.append(" Where cs.curso_id=c.id");
			sql.append(" AND s.id=cs.seccion_id");
			sql.append(" order by 1");
			
			
			/*
			 * select c.id,cs.abreviatura,s.nombre
				From t_curso c, t_curso_seccion cs , t_seccion s
				Where cs.curso_id=c.id
				AND s.id=cs.seccion_id
				order by 1
			 */
			...
			while (rs.next()) {
				curso= new Curso();
				curso.setCodigo(rs.getString("codigo").trim());
				curso.setCurso(rs.getString("curso").trim());
				curso.setSeccio(rs.getString("seccion").trim());
				curso.setCantidadAlumnos(ObtenerCantAlumnosSeccion(curso.getCodigo(),curso.getSeccio()));
				listado.add(curso);
			}
			return listado;
	...
	private int ObtenerCantAlumnosSeccion(String codigo,String seccion) throws Exception 
	{
		StringBuffer sql = null;
		...
		try {
			...
			sql.append(" And curso_id='"+ codigo +"'");
	...
	public List<Seccion> SeleccionarSeccionAgrupados(String idCurso) throws Exception {
		...
		{			
			query = new StringBuffer();
			query.append("SELECT c.id as codigo,c.nombre as curso,s.nombre as seccion");
			query.append(" FROM t_plan_curricular_detalle pd, t_horario_detalle hd, t_disponibilidad_aula da,t_aula a, t_dia d,t_hora h,t_curso_seccion cs,t_curso c,t_seccion s");
			query.append(" Where da.id=hd.disponibilidad_aula_id");
			query.append(" And pd.curso_id=c.id");
			query.append(" And da.aula_id=a.id");
	...
	public List<Horas> HorasSeccion(String idCurso, String seccion) throws Exception {
		...
		{			
			...
			query.append(" and cs.seccion_id=s.id");
			query.append(" And c.id=" +idCurso );
			query.append(" And s.nombre='" +seccion +"'" );
			query.append(" GROUP BY s.id,d.nombre");
			query.append(" ORDER BY 3,2 desc,h.turno desc,horaInicio");
			...
			listado= new ArrayList<Horas>();
			hora= new Horas();
											
			while (rs.next()) 
			{	
				hora= new Horas();
				hora.setDia(rs.getString("dia").toUpperCase());
				hora.setHoraInicio(rs.getString("horaInicio").trim().substring(0,5));
				hora.setHoraFin(rs.getString("horaFin").trim().substring(0,5));					
				listado.add(hora);							
			}
			...
			System.out.println("ERROR AL VERIFICAR LAS HORAS DE SECCION DE UN ALUMNO=> " + e.getMessage());
			return null;
		} finally 
		{
			LimpiarConexion(con, query, ps, rs);
	...
	public List<CursoCruce> ListarCursosXCicloCruces() throws Exception {
		...
		try 
		{
			...
			*/
			...
			query.append(" select cr.idCurso1 as codigoCurso1,c.nombre as curso1,c1.nombre as ciclo1,cr.seccion1,");
			query.append(" cr.idCurso2 as codigoCurso2,c2.nombre as curso2,ci2.nombre as ciclo2, cr.seccion2,count(*) as cantidadCruce");
			query.append(" FROM t_cruce cr ,t_curso c,t_curso c2, t_plan_curricular_detalle pc,t_plan_curricular_detalle pc2,");
			query.append(" t_ciclo c1,t_ciclo ci2");
			query.append(" Where c.id=cr.idCurso1");
			query.append(" AND pc.curso_id=c.id");
			query.append(" AND pc.ciclo_id=c1.id");
			query.append(" AND c2.id=cr.idCurso2");
			query.append(" AND pc2.curso_id=c2.id");
			query.append(" AND pc2.ciclo_id=ci2.id");
			query.append(" GROUP BY cr.idCurso1,cr.seccion1,cr.idCurso2,cr.seccion2");
