...
@WebServlet("/ListarCorreos")
public class ListarCorreos extends ActionServlet {
	...
	public void SimularCursosConcluyente() throws Exception {
		...
		try 
		{
			...
			listaArea = factory.getSimulacionMatricula().SimulacionConcluyente(condicion);
			
			System.out.println("LISTA DEL AREA Y CURSOS"+ listaArea);
						
			request.setAttribute("listaCursos", listaArea);
			*/
			request.getRequestDispatcher("SimulacionMatricula/ListarCorreos.jsp").forward(request, response);
